...
def Barcode_crop(image,angle, polygon,th=20):
    # 이미지를 불러옵니다.
    # 다각형의 좌표를 이용해 시작점, 너비, 높이를 얻습니다.
    sx, sy, w, h = get_bbox_from_polygon(polygon)
    absth1 = 0
    ...
    if sx < 0 :
        ...
        absth2 = abs(sy) + 30
        sy = 0
    image = add_padding(image,absth2,absth2,absth1,absth1)
    image = IMP.Tool.Rotate(image,angle)
    if sx == 0 :
        cropped_image = image[sy-th-absth2:sy+h+th+absth2, sx:sx+w+th+absth1]
    elif sy == 0:
        cropped_image = image[sy:sy+h+th+absth2, sx-th:sx+w+th+absth1]
    else :
        cropped_image = image[sy-th:sy+h+th, sx-th:sx+w+th]
    return cropped_image
...
### 라벨지와 바코드 위치 비교
def compare_bbox_positions(image,bar_center, label_center):
    bbox1_x, bbox1_y = bar_center
    bbox2_x, bbox2_y = label_center
...
def compare_bbox_positions_except(image,bar_center, label_center):
    bbox1_x, bbox1_y = bar_center
    bbox2_x, bbox2_y = label_center
    h,w = image.shape[:2]
...
## OCR처리를 2번 하지 않게 하기위해서 바코드 이력번호와 라벨지 이미지를 합치기
def combine_images(image1, image2):
    # 이미지1과 이미지2의 크기 가져오기
    h1, w1, _ = image1.shape
    h2, w2, _ = image2.shape
    # 이미지2의 너비를 이미지1의 너비에 맞추기
    resized_image2 = cv2.resize(image2, (w1, h2))
    # 이미지2의 높이 가져오기
    _, w2_resized, _ = resized_image2.shape
    # 이미지1과 이미지2 이어붙이기
    result = np.vstack((image1, resized_image2[:, :w1]))
    return result
...
def main() :
    ...
    for file in flist :
        ...
        results = model.predict(gray_path,save=False, imgsz=640, conf=0.7,name=f'd:/yolo/Inf_result/',save_txt = False,device=0)
        barcode_image = []
        ...
        try : 
            ...
            if sum(cnt) > 2 :
                if cnt[1] > 0 and cnt[2] > 0 :
                    ...
                    cv2.imwrite(f"{result_path}{file[:4]}_Fail",src)
                    print("Fail")
                    continue 
            endtime = time.time()
            print("polygon 보정 처리 시간 : ",round(endtime-start,3))
            ...
            for idx,box in enumerate(boxes) :
                if cnt[idx] > 0 :
                    ...
                    centers.append(p)
                    if idx == 0 :
                        rotate_src = IMP.Tool.Rotate(src,angle)
                        rotate_image = crop_image(rotate_src,replace_box,20)
                    elif idx == 2 :
                        if rotate_image is not None :
                            try :
                                barcode_image = Barcode_crop(src,angle,replace_box,40)
                            except :
                                barcode_image = Barcode_crop(src,angle,replace_box,10)
                else :
                    pass
            endtime = time.time()
            if len(centers) == 2 :
                if rotate_image.shape[0] > rotate_image.shape[1] * 5 or rotate_image.shape[1] > rotate_image.shape[0] * 5 :
                ...
                if len(barcode_image) > 0 :
                    barcode_image = imutils.rotate_bound(barcode_image,pos)
                    rotate_image = combine_images(rotate_image,barcode_image)
                endtime = time.time()
                print("후 처리 시간 : ",round(endtime-start,3))
            ...
            run(f"{result_path}{file[:-4]}_crop.jpg")
        except :
            # cv2.imwrite(f"{result_path}{file[:-4]}_Fail.jpg",src)  
            pass
        endtime = time.time()
